...
class _BookingRequestScreenState
  ...
  // Sends push notification to landlord
  // ─────────────────────────────────────────────
  Future<void> _sendBookingRequest() async {
    ...
    try {
      ...
      final landlordId = widget.listing['landlordId'] ?? '';
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Review and confirm your request',
              style: GoogleFonts.poppins(
            ...
            child: Column(
              children: [
                _buildDetailRow('Monthly Rent',
                    'LKR ${_formatPrice(price)}', isLast: false),
                _buildDetailRow('Advance Payment',
                    'LKR ${_formatPrice(price)}',
                    subtitle: '1 month advance', isLast: false),
                _buildDetailRow('Total Due Now',
                    'LKR ${_formatPrice(price)}',
                    isTotal: true, isLast: true),
              ],
            ),
          ),
        ],
  ...
  Widget _buildDetailRow(String label, String value,
      {String? subtitle, bool isTotal = false, bool isLast = false}) {
    return Container(
      padding: const EdgeInsets.symmetric(
  ...
  Widget _buildConfirmButton() {
    return Container(
      ...
      decoration: const BoxDecoration(
        color: Colors.white,
        border: Border(top: BorderSide(color: Color(0xFFDDE3F0))),
      ),
